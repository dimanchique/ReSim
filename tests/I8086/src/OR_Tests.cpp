...
#include "I8086_SingleOpTests.h"
 ...
 class I8086_OR_Fixture : public I8086_DoubleOpFixture {};
 class I8086_OR_IM_Fixture : public I8086_SingleOpFixture {};

// Mem (BX addressed) <-- AX | Mem (BX addressed)
TEST_F(I8086_OR_Fixture, OR_Ev_Gv_BX_Addressed_AX) {
    ...
    TestMemRegInstruction(memAddress, memValue, OR_Ev_Gv, modReg, 16);
...
TEST_F(I8086_OR_Fixture, OR_Ev_Gv_Direct_Addressed_BX) {
    ModRegByteConstructor modReg;
...
TEST_F(I8086_OR_Fixture, OR_Ev_Gv_BP_Addressed_WithDisp_DX) {
    ModRegByteConstructor modReg;
...
TEST_F(I8086_OR_Fixture, OR_Ev_Gv_BXSI_Addressed_WithDisp_AX) {
    ModRegByteConstructor modReg;
...
// AX <-- AX | BX
TEST_F(I8086_OR_Fixture, OR_Gv_Ev_AX_BX) {
    cpu.AX = 0x0060;
    cpu.BX = 0x009A;
    ...
    TestRegRegInstruction<WORD>(&leftReg, &rightReg, OR_Gv_Ev, 16);

    EXPECT_EQ(cpu.AX, refValue);
}

// BX <-- BX | DI
TEST_F(I8086_OR_Fixture, OR_Gv_Ev_BX_DI) {
    cpu.BX = 0x0060;
    cpu.DI = 0x009A;
...
// AL <-- AL | BH
TEST_F(I8086_OR_Fixture, OR_Gb_Eb_AL_BH) {
    cpu.AL = 0x60;
    cpu.BH = 0x9A;
    ...
    TestRegRegInstruction<BYTE>(&leftReg, &rightReg, OR_Gb_Eb, 16);

    EXPECT_EQ(cpu.AL, refValue);
}

// BL <-- BL | CL
TEST_F(I8086_OR_Fixture, OR_Gb_Eb_BL_CL) {
    cpu.BL = 0x0060;
    cpu.CL = 0x009A;
