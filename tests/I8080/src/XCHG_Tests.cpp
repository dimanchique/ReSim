...
public:
    void XCHG_CanExchangeValues(const WORD initialValueD, const WORD initialValueH) {
        // given:
        cpu.wordToRegisterSwapped(initialValueD, cpu.D, cpu.E);
        cpu.wordToRegisterSwapped(initialValueH, cpu.H, cpu.L);
        ...
        // then:
        const WORD swappedValueD = cpu.wordRegisterAsWordSwapped(cpu.D, cpu.E);
        const WORD swappedValueH = cpu.wordRegisterAsWordSwapped(cpu.H, cpu.L);
