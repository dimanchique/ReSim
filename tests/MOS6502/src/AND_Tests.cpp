...
public:
    ...
    void AND_ZPX_CanDoAND(BYTE initialValue, BYTE memoryValue) {
        ...
        cpu.A = initialValue;
        cpu.X = 0x10;
        mem[0xFFFC] = MOS6502_OpCodes::AND_ZPX;
        mem[0xFFFD] = 0x42;
...
TEST_F(MOS6502_ANDFixture, AND_INDX_CanDoAND) {
    ...
    mem[0xFFFC] = MOS6502_OpCodes::AND_INDX;    // read the 8 bit value from the next mem cell and add X
...
TEST_F(MOS6502_ANDFixture, AND_INDY_CanDoAND) {
    ...
    mem[0xFFFC] = MOS6502_OpCodes::AND_INDY;    // read the 8 bit value from the next mem cell
...
TEST_F(MOS6502_ANDFixture, AND_INDY_CanDoAND_WithExtraCycleOnPageCrossing) {
    ...
    mem[0xFFFC] = MOS6502_OpCodes::AND_INDY;    // read the 8 bit value from the next mem cell and add X
    mem[0xFFFD] = 0x02;                         // read the 16 bit Little Endian address from 0x0002-0x0003
    mem[0x0002] = 0x02;                         //
