...
TEST_F(MOS6502_CPXFixture, CPX_IM_CanCompare) {
    CM_IM_CanCompare(MOS6502_OpCodes::CPX_IM, cpu.X, 0x10, 0x4);
    EXPECT_TRUE(cpu.Status.C);
    EXPECT_FALSE(cpu.Status.Z);
}

TEST_F(MOS6502_CPXFixture, CPX_IM_CanAffectZeroFlag) {
    CM_IM_CanCompare(MOS6502_OpCodes::CPX_IM, cpu.X, 0x10, 0x10);
    EXPECT_TRUE(cpu.Status.C);
    EXPECT_TRUE(cpu.Status.Z);
}

TEST_F(MOS6502_CPXFixture, CPX_IM_CanAffectNegativeFlag) {
    CM_IM_CanCompare(MOS6502_OpCodes::CPX_IM, cpu.X, 0x10, 0x11);
...
TEST_F(MOS6502_CPXFixture, CPX_ZP_CanCompare) {
    CM_ZP_CanCompare(MOS6502_OpCodes::CPX_ZP, cpu.X, 0x10, 0x4);
    EXPECT_TRUE(cpu.Status.C);
    EXPECT_FALSE(cpu.Status.Z);
}

TEST_F(MOS6502_CPXFixture, CPX_ABS_CanCompare) {
    CM_ABS_CanCompare(MOS6502_OpCodes::CPX_ABS, cpu.X, 0x10, 0x4);
    EXPECT_TRUE(cpu.Status.C);
    EXPECT_FALSE(cpu.Status.Z);
}
