...
TEST_F(MOS6502_LDXFixture, LDX_IM_CanLoadValue) {
    LD_IM_CanLoadValue(MOS6502_OpCodes::LDX_IM, cpu.X);
}

TEST_F(MOS6502_LDXFixture, LDX_IM_CanAffectZeroFlag) {
    LD_IM_CanAffectZeroFlag(MOS6502_OpCodes::LDX_IM);
}

TEST_F(MOS6502_LDXFixture, LDX_IM_CanAffectNegativeFlag) {
    LD_IM_CanAffectNegativeFlag(MOS6502_OpCodes::LDX_IM);
}

TEST_F(MOS6502_LDXFixture, LDX_ZP_CanLoadValue) {
    LD_ZP_CanLoadValue(MOS6502_OpCodes::LDX_ZP, cpu.X);
}

TEST_F(MOS6502_LDXFixture, LDX_ZPY_CanLoadValue) {
    cpu.Y = 0x5;
    LD_ZP_CanLoadValue(MOS6502_OpCodes::LDX_ZPY, cpu.X, cpu.Y);
}
...
TEST_F(MOS6502_LDXFixture, LDX_ABS_CanLoadValue) {
    LD_ABS_CanLoadValue(MOS6502_OpCodes::LDX_ABS, cpu.X);
}

TEST_F(MOS6502_LDXFixture, LDX_ABSY_CanLoadValue) {
    cpu.Y = 0x01;
    LD_ABS_CanLoadValue(MOS6502_OpCodes::LDX_ABSY, cpu.X, cpu.Y);
}
