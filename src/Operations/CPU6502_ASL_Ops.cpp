...
void CPU6502_ASL_ZP(U32 &cycles, Memory &memory, CPU6502 &cpu) {
    ...
    memoryValue <<= 1;
    CPU6502::DoTick(cycles);
    CPU6502::WriteByte(cycles, memory, memoryValue, ZeroPageAddress);
...
void CPU6502_ASL_ABS(U32 &cycles, Memory &memory, CPU6502 &cpu) {
    ...
    memoryValue <<= 1;
    CPU6502::DoTick(cycles);
    CPU6502::WriteByte(cycles, memory, memoryValue, AbsAddress);
    cpu.Status.UpdateStatus(memoryValue, CPU6502_Status_Z | CPU6502_Status_N);
    cpu.Status.C = Carry;
}
...
void CPU6502_ASL_ABS(U32 &cycles, Memory &memory, CPU6502 &cpu, BYTE affectingRegister) {
    ...
    memoryValue <<= 1;
    CPU6502::DoTick(cycles);
    CPU6502::WriteByte(cycles, memory, memoryValue, AffectedAbsAddress);
    cpu.Status.UpdateStatus(memoryValue, CPU6502_Status_Z | CPU6502_Status_N);
    cpu.Status.C = Carry;
}
