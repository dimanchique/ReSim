...
for group_name, group_instructions_file in instructions_targets.items():
    ...
    scan_regex(
        group_instructions_file,
        opcode_re_exp,
        lambda x: opcodes_list.append(x.group(1)),
    )
    ...
    scan_regex(
        read_targets[group_name],
        opcode_re_exp,
        lambda x: op_map.update({x.group(1): int(x.group(2), 16)}),
    )

    print(f'--> Group "{group_name}": ', end="")
    if len(op_map) == 0:
        print("skipped, no marked instructions found")
        continue
    else:
...
"""
    ...
    with open(
        os.path.join(cpu_include_root, f"{cpu_name}_OpCodesList_{group_name}.h"), "w"
    ) as out_file:
...
print("----------------------------------------------------------------------------")
print(
    f"Used instructions: {total_used_instructions}/{detected_instructions} ({used_instructions_percentage:.2f}%)"
)
print()
